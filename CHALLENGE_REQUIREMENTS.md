# Pretty Good AI — AI Engineering Challenge (Official)
...
Setup: Create a test account at pgai.us/athena — this gives you context on how our
product works and what patients experience. Do not call the number shown on the
confirmation screen.
...
- Calls only our test number: +1-805-439-8008
- Simulates realistic patient scenarios (scheduling, refills, questions, etc.)
- Records and transcribes the conversations
- Identifies bugs or quality issues in our agent's responses

## Requirements

### Deliverables (all in GitHub)

1. Working code: Your voice bot, written in Python
2. README: Clear setup and run instructions (ideally a single command after setup)
3. Architecture doc: 1–2 paragraphs explaining how your system works and why you made key design choices
4. Call transcripts and recordings: Minimum 10 calls with both sides of each conversation. A good call is a full conversation (typically 1–3 minutes), not a single question and hang-up (MUST submit the audio recordings in OGG or MP3 format as well as the transcript)
5. Bug report: Document issues you found
6. Loom videos:
   - Walkthrough of your approach and what you built (max 3 minutes, free Loom account). This is one of the most important deliverables — show us how you think, and convince us that you are an exceptional communicator. Please make sure your video is on. This role requires frequent engagement with team members and customers over video conferences, so we want to see how you communicate, not just read it.
...
Should have informed the patient the office is closed on weekends and offered
the next available weekdays.
```
...
1. Your bot holds a coherent voice conversation with our agent. Submissions that don't clear this bar are rejected without further review.
...
3. Make sure these are ALL included:
   ...
   - 2 Loom videos (MUST BE PUBLIC) — 1) walkthrough of project & 2) AI debugging video
   - Videos are recorded in your own voice and use a webcam
   - The one phone number you used to call our bot during testing, in E.164 format (example: +13334445555). Only use a single number for all your test calls. (ENSURE THIS IS CORRECT OR WE CANNOT GRADE THE QUALITY OF SUBMISSION)

There is no hard deadline. Submissions are reviewed on a first-in, first-reviewed basis.

Please refrain from contacting us or any member of the Pretty Good AI team via email, LinkedIn, or other direct channels regarding this assessment.
